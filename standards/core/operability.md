...
- Review questions: Can a normal user or support person identify the running version, commit, and build from the product UI in production? If the visible commit is shortened, is the exact commit available somewhere appropriate for the product's support flow? Do missing fields render as `Unavailable` instead of disappearing? Is the provenance shown in stable product chrome rather than devtools, hidden gestures, or environment-specific debug modes? Would a copy affordance help support, or would it add low-value chrome?
- Automation potential: Build pipelines and static checks can verify some metadata wiring, but visibility, placement, optional copy affordances, and scope judgment still require human review.
