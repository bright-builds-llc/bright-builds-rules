...
- let the managed `.githooks/pre-commit` starter-check hook run `bun scripts/bright-builds-check.ts all`
```
...
- Exceptions or escape hatches: If repo-local guidance already says that hooks are advisory, mandatory, or intentionally incomplete, follow that documented workflow instead of asking again. The managed `.githooks/pre-commit` starter-check hook is a commit gate. It does not replace running `bun scripts/bright-builds-check.ts all` before commit, and `--no-verify` is not an acceptable skip.
