...
- Fully managed downstream files such as `AGENTS.bright-builds.md`, the PR template, the audit trail, and enabled auto-update files use visible whole-file managed markers and should block status/update when they drift downstream. `CONTRIBUTING.md` now uses a bounded managed block and should block only when that managed block drifts or breaks.
