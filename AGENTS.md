...
- remember that install/update now copy the managed standards corpus under downstream `standards/`
...
- Keep managed downstream Markdown clean under the repository's pinned Python 3.13+ formatter stack: `mdformat==1.0.0`, `mdformat-frontmatter==2.1.2`, and `mdformat-gfm==1.0.0`. `./scripts/verify-docs.sh` rejects a bare or incompatible PATH formatter and treats formatting drift in managed downstream Markdown as an installer/template regression.
- Bright Builds owns only its managed Markdown. Install and update must preserve downstream `.mdformat.toml` files and arbitrary user-authored Markdown; repository-specific migrations, including GSD marker migration, stay with the downstream repository's native tooling.
- Treat `.codex/tasks/lessons.md` as this repository's active lesson ledger. Before substantive plan, review, implementation, or audit work, load and maintain it under the bounded context policy in `standards/core/local-guidance.md`; lesson audit and archive files are operational metadata and are not normal startup context.
...
- `bun run context-cost:update` owns `metrics/context-cost-history.csv` and the guarded README context-cost block. `bun run hooks:install` replaces the repo-local `core.hooksPath` with `.githooks`; when a refresh is needed, its pre-commit hook stages the complete current skill, adoption guide, README, and context-cost history files.
- Downstream install and update write the managed starter-check hook at `.githooks/pre-commit` and set that repo's local `core.hooksPath` to `.githooks`. That hook is separate from this repository's context-cost pre-commit hook. The Bright Builds Rules skill confirms that activation after install or update, and sets `core.hooksPath` once on a later clone when the managed hook is present and the local config is unset.
- Fully managed downstream files such as `AGENTS.bright-builds.md`, the PR template, the audit trail, the local standards corpus, and enabled auto-update files should block status/update when they drift downstream. Marker-bearing managed files use visible whole-file managed markers; copied standards files remain exact canonical Markdown without marker comments. `CONTRIBUTING.md` now uses a bounded managed block and should block only when that managed block drifts or breaks.
