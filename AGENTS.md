...
- Downstream install and update write the managed starter-check hook at `.githooks/pre-commit` and set that repo's local `core.hooksPath` to `.githooks`. That hook is separate from this repository's context-cost pre-commit hook.
